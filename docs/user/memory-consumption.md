...
> Researched against upstream torii 1.8.6. Defaults below are verified against
> `crates/cli/src/options.rs` and `crates/sqlite/sqlite/src/constants.rs`. This build differs in
> two ways that matter here: the gRPC keepalive settings also apply to the public HTTP listener, so
> dead clients are torn down instead of pinning their subscriptions, and closed gRPC subscribers
> are swept every 30 seconds rather than on the next update. For telling retention from a leak on
> a running server, see [`memory-diagnostics.md`](./memory-diagnostics.md).
...
Watch RSS over a week. Healthy pattern: a high plateau during initial sync, settling to a stable steady-state at chain tip. A slow continuous climb at tip points at subscription leakage (slow clients), allocator retention, or the ABI/token caches. RSS alone cannot separate those; use the `torii_memory_*`, `torii_grpc_subscribers` and `torii_proxy_connections` gauges described in [`memory-diagnostics.md`](./memory-diagnostics.md).
